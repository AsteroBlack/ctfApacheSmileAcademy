...
@Component({
  selector: 'app-demande-acceptee',
  templateUrl: './demande-acceptee.component.html',
  styleUrls: ['./demande-acceptee.component.scss']
})
export class DemandeAccepteeComponent implements OnInit {
      ...
      .pipe(
        tap((res) => {
          if(!res['hasError'] && res['items']){
            const idDemande= res['items'].filter(item => item.code === 'ACCEPTER')[0].id
