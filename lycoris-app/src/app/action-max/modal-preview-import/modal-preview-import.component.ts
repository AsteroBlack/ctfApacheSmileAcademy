...
export class ModalPreviewImportComponent implements OnInit {
  ...
  getItems(param?: any) {
    let request = {
      user: this.user.id,
      data: {
        createdAt: param?.start ? param.start : moment().format('DD/MM/YYYY'),
        createdAtParam: {
          operator: '[]',
          start: param?.start ? param.start : moment().format('DD/MM/YYYY'),
          end: param?.end ? param.start : moment().format('DD/MM/YYYY')
        }
      },
