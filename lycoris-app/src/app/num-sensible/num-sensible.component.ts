...
import { ResponseTypeNumero, TypeNumeroLabel } from '../core/models/type-numero.model';
import { map, tap, switchMap, catchError } from 'rxjs/operators';
import { Observable, ReplaySubject, of } from 'rxjs';
import { TypeNumeroService} from '../core/service/type-numero.service';
import { MachineInfoService } from '../core/service/machine-info.service';

@Component({
  selector: 'app-num-sensible',
  templateUrl: './num-sensible.component.html',
  styleUrls: ['./num-sensible.component.scss'],
})
export class NumSensibleComponent implements OnInit {
  items: Array<any> = [];
  ...
  filterItems() {
    ...
    this.items = this.items.filter(item => {
      ...
      const fields = [
        (item.userLogin || "").toLowerCase(),
        item.numero || "",
        (item.categoryCode || "").toLowerCase(),
        (item.contractId || "").toLowerCase(),
        (item.offerName || "").toLowerCase(),
        (item.portNumber || "").toLowerCase(),
        (item.serialNumber || "").toLowerCase(),
        (item.statusBscs || "").toLowerCase(),
        (item.statuMachine || "").toLowerCase(),
        ("PAS EN MACHINE" || "MISE EN MACHINE").toLowerCase()
      ]

      if(searchTermLower.includes("bloque") || searchTermLower.includes("debloque")){
        console.log("searchTermLower.match('bloque') :" +searchTermLower.match("bloque"), "searchTermLower.match('debloque') :"+searchTermLower.match("debloque"))
        console.log("searchTermLower", searchTermLower)
        console.log("searchTermLower.includes('bloque')", searchTermLower.includes("bloque"))
        console.log("searchTermLower.includes('debloque')", searchTermLower.includes("debloque"))
        console.log("fields.includes('debloque')", fields.includes("debloque"))
        console.log("fields.includes(searchTermLower)", fields.includes(searchTermLower))
        console.log("fields.some(field => field.match(searchTermLower))", fields.some(field => field.match(searchTermLower)));
        console.log("fields.some(field => field.includes(searchTermLower))", fields.some(field => field.includes(searchTermLower)));
        console.log("fields.some(field => field == searchTermLower)", fields.some(field => field == searchTermLower))
        console.log("fields.some(field => field === searchTermLower)", fields.some(field => field === searchTermLower))
        return fields.some(field => field === searchTermLower);
      }
 
      console.log(fields.some(field => field.includes(searchTermLower)));
      return fields.includes(searchTermLower);
    });
